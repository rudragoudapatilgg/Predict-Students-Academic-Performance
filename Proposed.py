...
# Improved Cock-hen-chicken Optimizer (ICHC) Strating Line No. 58
def Proposed(positions, fobj, VRmin, VRmax, maxIter):
    ...
    for t in range(maxIter):
        ...
        for idx in cocks:
            currfitness = fitness[idx]
            bestfit = np.min(fitness)
            worstfit = np.max(fitness)
            r1 = (currfitness - bestfit) / (worstfit + bestfit)
