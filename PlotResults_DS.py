...
def plot_ConvResults():
    ...
    plt.legend(loc=1)
    plt.savefig("./Results/Conv_2.png")
    plt.show()


def Dataset_2_Table():
    eval = np.load('Eval_Rate.npy', allow_pickle=True)
    ...
    for i in range(eval.shape[0]):
        for k in range(eval.shape[1]):
            ...
            print('-------------------------------------------------- Dataset', 1 + 1, 'Learning_Rate',
                  Learning_Rate[k],
...
def PlotsResults():
    ...
    for i in range(eval.shape[0]):
        for j in range(len(Graph_Terms)):
            ...
            path = "./Results/Dataset_%s_%s_line.png" % (1 + 1, Terms[Graph_Terms[j]])
            plt.savefig(path)
            plt.show()
            ...
            plt.xlabel('Epoch', fontname="Arial", fontsize=12, fontweight='bold', color='k')
            plt.ylabel(Terms[Graph_Terms[j]], fontname="Arial", fontsize=12, fontweight='bold', color='k')
            path = "./Results/Dataset_%s_%s_bar.png" % (1 + 1, Terms[Graph_Terms[j]])
            plt.savefig(path)
            plt.show()


def PlotResults():
    Eval = np.load('Eval_Summ_2.npy', allow_pickle=True)
    ...
    print('-------------------------------------------------- Dataset-' + str(
        1 + 1) + ' Algorithm Comparison(Rouge-1) ',
          ' --------------------------------------------------')
    print(Table)
    ...
    print('-------------------------------------------------- Dataset-' + str(
        1 + 1) + ' Methods Comparison(Rouge-1) ',
          ' --------------------------------------------------')
    print(Table)
    ...
    for k in range(len(Rouge)):
        ...
        path = "./Results/Dataset_%s_%s_Algorithm.png" % (str(1 + 1), Rouge[k])
        plt.savefig(path)
        plt.show()
        ...
        plt.xticks(X + 0.30, ('F1 Score', 'Precision', 'Recall'), fontname="Arial", fontsize=12, fontweight='bold',
                   color='k')
        plt.ylabel(Rouge[k], fontname="Arial", fontsize=12, fontweight='bold', color='k')
        path = "./Results/Dataset_%s_%s_Method.png" % (str(1 + 1), Rouge[k])
        plt.savefig(path)
        plt.show()
